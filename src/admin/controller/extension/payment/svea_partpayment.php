...
class ControllerExtensionPaymentSveapartpayment extends Controller
{
    ...
    public function index()
    {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/payment/svea_partpayment', 'token=' . $this->session->data['token'], true)
        );


        $data['action'] = $this->url->link('extension/payment/svea_partpayment', 'token=' . $this->session->data['token'] . '&type=payment', true);
        ...
        $this->response->setOutput($this->load->view('extension/payment/svea_partpayment.tpl', $data));
