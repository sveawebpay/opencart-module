...
class ControllerExtensionTotalSveaFee extends Controller
{
    ...
    public function index()
    {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/total/svea_fee', 'token=' . $this->session->data['token'], true),
            'separator' => ' :: '
        );

        // actions urls for the save/cancel buttons
        $data['action'] = $this->url->link('extension/total/svea_fee', 'token=' . $this->session->data['token'], true);
        ...
        $this->response->setOutput($this->load->view('extension/total/svea_fee.tpl', $data));
    }
    ...
    {
        return $this->url->link($link, 'token=' . $this->session->data['token'] . '&type=total', 'SSL');
    }
}
