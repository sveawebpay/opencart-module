...
class ControllerSveaSyncOrder extends Controller
{
    protected $current_order_id;

    public function index()
    {
        $this->load->language('svea/sync_order');
        $json = array();
...
//        $this->updateOCOrder($checkout_order);
    ...
    private function getOrderStatusId($checkout_order)
    {
        ...
        $sco_status = $checkout_order['orderstatus'];
        ...
        switch ($sco_status) {
            ...
            case 'Delivered':
                if ($this->isCredited($checkout_order) === true) {
                    $oc_order_status_id = $this->config->get('sco_credited_status_id');
                } else {
                    $oc_order_status_id = $this->config->get('sco_delivered_status_id');
                }
                break;
            case 'Cancelled':
                $oc_order_status_id = $this->config->get('sco_canceled_status_id');
                break;
            case 'Pending':
    ...
    private function isCredited($checkoutOrder)
    {
        $isCredited = false;

        $orderAmount = $checkoutOrder['orderamount'];
        $cancelledAmount = $checkoutOrder['cancelledamount'];
        $creditedAmount = 0;

        $deliveries = $checkoutOrder['deliveries'];
        foreach ($deliveries as $delivery) {
            if ($delivery['deliveryamount'] === $delivery['creditedamount'] && $delivery['deliveryamount'] > 0) {
                $creditedAmount += $delivery['creditedamount'];
            }
        }

        $total = $orderAmount - $cancelledAmount - $creditedAmount;
        if ($total === 0 && $creditedAmount > 0) {
            $isCredited = true;
        }
