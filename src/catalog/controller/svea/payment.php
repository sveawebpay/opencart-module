...
class ControllerSveaPayment extends SveaCommon
{
    public function index()
    {
        ...
        try {
            ...
            $this->response->setOutput($this->load->view('svea/payment', $data));
        } catch (\Exception $e) {
    ...
    private function setOrderGeneralData($checkout_order_entry, $locale)
    {
        $checkout_order_entry
            ...
            ->setTermsUri($this->url->link('information/information', array('information_id' => $this->config->get('config_checkout_id'))))
            ->setLocale($locale);
    }
