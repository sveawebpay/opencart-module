    ...
    public function history()
    {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            ...
            if ($payment_method == 'svea_invoice'
                ...
                if ($request_order_status_id == $this->config->get($payment_method . '_canceled_status_id')
                    && $request_order_status_id != $this->config->get($payment_method . '_canceled_status_id')
                ) {
                ...
                if ($this->config->get($payment_method . '_testmode' . $testmode_countrycode) !== NULL) {
                    $conf = ($this->config->get($payment_method . '_testmode' . $testmode_countrycode) == "1")
                ...
                if ($this->config->get($payment_method . '_canceled_status_id') == $request_order_status_id
                    && $payment_method != 'svea_directbank'
                ) {
                ...
                } elseif ($this->config->get($payment_method . '_deliver_status_id') == $request_order_status_id
                    && $payment_method != 'svea_directbank'
                ) {
                    try {
                        $credit_response = \Svea\WebPay\WebPay::deliverOrder($conf)
                            ...
                            ->setCountryCode($order_info['payment_iso_code_2']);
                        if ($payment_method == 'svea_invoice') {
                            $svea_response = $credit_response->setInvoiceDistributionType($this->config->get($payment_method . '_distribution_type'))
                                ->deliverInvoiceOrder()
                                ->doRequest();
                        } else if ($payment_method == 'svea_partpayment') {
                ...
                } elseif ($this->config->get($payment_method . '_refunded_status_id') == $request_order_status_id) {
                        ...
                        if (sizeof($invoiceId) <= 0 && sizeof($transactionId) <= 0 && sizeof($contractNumber) <= 0) {
                            ...
                            if ($payment_method == 'svea_invoice') {
                                $credit_response = $credit_response
                                    ...
                                    ->setInvoiceDistributionType($this->config->get($payment_method . '_distribution_type'))
                ...
                $order_info = $this->model_checkout_order->getOrder($order_id);
                $sco_order = $this->model_extension_svea_checkout->getCheckoutOrder($order_id);
                ...
                if ($sco_order !== null && isset($sco_order['checkout_id'])) {
                    ...
                    $country = $this->model_extension_svea_checkout->getCountryCode($sco_order['checkout_id']);
                    ...
                    if ($this->config->get('sco_canceled_status_id') === $order_status_id) {
                        ...
                        try {
                            if ($this->canOrderBeCreditedByDeliveryRows($sco_order_id, $country) === true) {
                                ...
                                if (isset($credit_response['HeaderLocation'])) {
                                    $history_comment = 'Order credited at Svea.' . ' Svea Checkout Order Id: ' . $sco_order_id;
                                    $svea_comment = ' | Order credited at Svea. Svea Checkout Order Id: ' . $sco_order_id;
                                    $this->model_checkout_order->addOrderHistory($order_id, $order_status_id, $history_comment);
                                } else {
                        ...
                        try {
                            if ($this->canOrderBeDelivered($sco_order_id, $country) === true) {
                                ...
                                if (isset($deliver_response['HeaderLocation'])) {
                                    $history_comment = 'Order delivered at Svea.' . ' Svea Checkout Order Id: ' . $sco_order_id;
                                    $svea_comment = ' | Order delivered at Svea. Svea Checkout Order Id: ' . $sco_order_id;
                                    $this->model_checkout_order->addOrderHistory($order_id, $order_status_id, $history_comment);
                                } else {
