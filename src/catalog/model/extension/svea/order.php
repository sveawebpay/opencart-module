...
class ModelExtensionSveaOrder extends Model
{
    public function deliverOrder($config, $paymentMethod, $sveaOrderId, $countryCode)
    {
        ...
        try {
            ...
            if ($paymentMethod == "svea_invoice") {
                $response = $response->setInvoiceDistributionType($this->config->get($paymentMethod . '_distribution_type'))
                    ->deliverInvoiceOrder()
                    ->doRequest();
    ...
    public function getConfiguration($paymentMethod, $countryCode)
    {
        if ($paymentMethod == "sco")
        {
            $config = ($this->config->get('sco_test_mode') == '1') ? new OpencartSveaCheckoutConfigTest($this->config, 'checkout') : new OpencartSveaCheckoutConfig($this->config, 'checkout');
        } else {
            $configCountryCode = $paymentMethod == "svea_invoice" || $paymentMethod == "svea_partpayment" ? '_' . $countryCode : '';

            if ($this->config->get($paymentMethod . "_testmode" . $configCountryCode) !== NULL) {
                $config = ($this->config->get($paymentMethod . "_testmode" . $configCountryCode) == "1") ? new OpencartSveaConfigTest($this->config) : new OpencartSveaConfig($this->config);
            }
        }
        return $config;
    }
